...
- [ ] **Task 26** — Real-time push to dashboard via Server-Sent Events. **A real bug was found via manual browser testing after initial implementation — see 26.5 and `DECISIONS.md`'s bug-fix entry.** **Auth note (see addendum):** native `EventSource` cannot send an `Authorization` header, and this app's `morgan` logging means a JWT-in-query-string would land in plaintext server logs — so auth is a short-lived, single-use SSE ticket, not the JWT directly. Sub-parts:
  ...
  - [ ] 26.5 — Manual test (two browser tabs, confirm live push) + docs + commit. **History:** first checked off on too narrow a basis (a Node-script-level check, no real browser), corrected back to unchecked. User then ran the real two-tabs test three times, each time reporting exactly what did and didn't sync: (1) found the live indicator and all updates broken — root cause was the Redis connection hanging forever when unreachable, fixed; (2) still broken after Redis was confirmed reachable — root cause was a genuinely missing route registration for `POST /:id/sse-ticket`, fixed; (3) status changes and new-group creation now confirmed syncing live for real, but duplicate/count-bump events (an existing group hit again) never synced — this was a deliberately-scoped-out case from Task 26's original design, now built (`duplicate_recorded` event type). **Still needs:** one more quick confirmation specifically of the duplicate/count-bump fix (simulate an error against an existing group, confirm the count updates live in a second tab) — everything else this sub-task asks for has now been genuinely confirmed working by a real person in a real browser.
