...
**Post-ship polish — stuck AI enrichment re-queued, source-map upload UI added.**
- **Re-enrichment.** The real Gemini free-tier quota (exhausted during earlier Task 38 load-testing, see the `seedTestProjects.js` orphan-cascade bug entry in `DECISIONS.md`) had left 8 real error groups with `aiSummary: null` after their enrichment jobs exhausted retries. Once the daily quota reset, all 8 were re-enqueued through the real production `enrichmentQueue` (not a workaround path) and completed — 6 immediately, 2 more after clearing a real per-minute rate limit via BullMQ's normal retry/backoff. All 70 error groups in the database now have AI analysis.
- **Source-map upload UI.** Task 32 shipped API-only (`POST/GET/DELETE /api/projects/:id/sourcemaps`, no client) — closed with a new `components/project-detail/SourceMapManager.jsx`, a drawer on `ProjectDetailPage` (same expandable-panel pattern as `SdkSnippetGenerator`): upload a `.map` file (bundle filename + optional release), list what's uploaded, delete. Live-verified against the real API: upload → `201`, list refresh, delete → `200`, plus both validation layers (client-side "is this even JSON" and the server's Source Map v3 schema check) confirmed surfacing clear errors through the UI. One real finding during testing: the Browser automation tool's coordinate/ref-based click didn't register on the toggle button (confirmed via direct DOM `.click()` that the component itself was correct) — a tooling quirk, not a code bug, noted here in case it recurs.
- **Tests.** No new automated tests this pass (data re-enqueue is an operational action, not code; the UI was verified live rather than unit-tested, consistent with how `SdkSnippetGenerator` and other presentational components in this codebase are handled). Full server suite: 82/82 passing, unaffected. Client production build clean.
